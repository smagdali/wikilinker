...
The extension isn't in the Chrome Web Store or Firefox Add-ons yet — for now you can [download the latest release](https://github.com/smagdali/wikilinker/releases/tag/v0.5.2) and install it manually. In Chrome, unzip and load it via `chrome://extensions` with Developer Mode enabled. In Firefox, use `about:debugging` to load it as a temporary add-on.
